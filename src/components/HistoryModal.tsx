...
export const HistoryModal: React.FC<Props> = ({ history, isLoading, onClose, onEdit, onDelete, onDownload }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[100] p-4 backdrop-blur-sm transition-all">
      <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col border border-gray-100">
        ...
        <div className="space-y-3 overflow-y-auto pr-2 flex-1">
          {isLoading ? (
            ...
            history.map((invoice, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-blue-200 transition-all gap-4">
                <div>
                  <p className="font-semibold text-gray-900">{invoice.clientName || 'Unknown Client'}</p>
                  <p className="text-sm text-gray-500">#{invoice.invoiceNumber} • {invoice.date}</p>
                </div>
